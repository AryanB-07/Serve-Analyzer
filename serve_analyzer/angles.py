...
def joint_angle(a: np.ndarray, b: np.ndarray, c: np.ndarray) -> np.ndarray:
    ...
    Inputs are (..., 2) arrays. Returns NaN where any point is missing or
...
def tilt_from_vertical(bottom: np.ndarray, top: np.ndarray) -> np.ndarray:
    """Unsigned angle in degrees between the bottom->top segment and image-up."""
    up = np.zeros_like(np.asarray(bottom, float))
    up[..., 1] = -1.0  # image y grows downwards
...
def compute_series(seq: PoseSequence, hand: Hand) -> Series:
    ...
    def knee_flexion(leg: L.LegSide) -> np.ndarray:
        return 180.0 - joint_angle(p(leg.hip), p(leg.knee), p(leg.ankle))

    return {
        FRONT_KNEE_FLEXION: knee_flexion(front),
        BACK_KNEE_FLEXION: knee_flexion(back),
        ELBOW_ANGLE: joint_angle(p(hit.shoulder), p(hit.elbow), p(hit.wrist)),
